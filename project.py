...
response_active = response_dadata(data_active, API_URL, headers)
response_liquidating = response_dadata(data_liquidating, API_URL, headers)
response_liquidated = response_dadata(data_liquidated, API_URL, headers)
response_bankrupt = response_dadata(data_bankrupt, API_URL, headers)
response_suspended = response_dadata(data_suspended, API_URL, headers)
response_reorganizing = response_dadata(data_reorganizing, API_URL, headers)
...
resultation(df_active, 'companies_active_result', cities, regions)
resultation(df_liquidating, 'companies_liquidating_result', cities, regions)
resultation(df_liquidated, 'companies_liquidated_result', cities, regions)
resultation(df_bankrupt, 'companies_bankrupt_result', cities, regions)
resultation(df_suspended, 'companies_suspended_result', cities, regions)
resultation(df_reorganizing, 'companies_reorganizing_result', cities, regions)